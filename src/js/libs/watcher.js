...
class ScrollWatcher {
  ...
  scrollWatcherConstructor(items) {
    if (items.length) {
      ...
      const uniqParams = uniqArray(Array.from(items).map((item) => `${item.dataset.watchRoot ? item.dataset.watchRoot : null}|${item.dataset.watchMargin ? item.dataset.watchMargin : '0px'}|${item.dataset.watchThreshold ? item.dataset.watchThreshold : 0}`));
