export const showMore = (function () {
  ...
  const htmlSubstr = (originalText, count) => {
    const div = createElementTemplate('div');
  ...
  class ShowMore {
    constructor(className, _temp) {
      ...
      this._createBtn = (_ref) => {
        ...
        const button = createElementTemplate('button');
      ...
      this._handleEvent = (object, _ref2) => {
        ...
        if (type === 'text' && checkContainsClass) {
          ...
          if (less) {
            const el = createElementTemplate(typeElement);
      ...
      this._getNumberCount = (element, type) => type === 'list' ? [].slice.call(element.children) : element.rows;
      this._addBtn = (object) => {
        const {
          type,
          element,
