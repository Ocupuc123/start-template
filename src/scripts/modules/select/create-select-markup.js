const createNativeOptionsMarkup = (items, activeIndex) => items.map((el, index) => {
...
export const createNativeSelectMarkup = ({id, items, multiple, name, required, activeIndex = []}) => `<select ${id ? `id='${id}'` : ''} ${name ? `name='${name}'` : ''} ${multiple ? 'multiple' : ''} ${
  required ? 'required' : ''
